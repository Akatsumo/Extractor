...
class VideoCryptExtractor:
    ...
    def get_content_url(self, course_id, content, headers, key, iv):
        ...
        if file_type == "3":
            if content.get("is_drm") == "1" or (not file_url and vdc_id):
                base = "https://abhinaymaths.in" if self.name == "abhinaymaths" else "https://www.videocrypt.in"
                urls.append(f"{base}/drm/{vdc_id}/{headers.get('userid')}")
            elif content.get("video_type") == "1" and content.get("file_url"):
                urls.append(f"https://youtu.be/{content['file_url']}")
            elif file_url:
                clean = file_url.replace("\\/", "/").replace("https\\:", "https:")
                urls.append(clean)
            ...
            if content.get("had_pdf") == "1":
                ...
                for pdf in result.get("data", []):
                    self.p_count += 1
                    urls.append(f"{pdf.get('pdf_title')} : {pdf.get('pdf_url')}")

        elif file_type in ("1", "7"):
            if file_url:
                clean = file_url.replace("\\/", "/").replace("https\\:", "https:")
                urls.append(clean)
            elif join_url:
                clean = join_url.replace("\\/", "/").replace("https\\:", "https:")
                urls.append(clean)
            if urls:
                self.p_count += 1
    ...
    def process_topic(self, course_id, batch_id, topic_id, subject_id, tile_type, tile_id, revert_api, headers, key, iv):
        ...
        for content in topic_data["data"]["list"]:
            if content.get("file_type") in ("1", "3", "7"):
                urls = self.get_content_url(course_id, content, headers, key, iv)
                if urls:
                    contents += f"{content['title']} : {urls}\n"
        return contents
        ...
        for tile in course_detail["data"]["tiles"]:
            ...
            for topic in tile["meta"]["list"]:
                for subtopic in topic["list"]:
                    results += self.process_topic(
                        course_id, batch_id,
                        subtopic["id"], topic["id"],
                        tile["type"], tile["id"],
                        tile["revert_api"], headers, key, iv
                    )
        return results
    ...
    async def start_login(self, app, message, user_id=None):
        ...
        try:
            ...
            mm = await message.reply_text(
                f"🕹 **Select {self.name.title()} Batches Mode 👇**\n\n"
                f"🔴 Without Purchased Batches\n🟢 Purchased Batches\n\n"
                f"**Notice**: Without Purchased mode allows access to all batches.",
                reply_markup=buttons
            )
            r = await mm.wait_for_click(from_user_id=user_id)
            await mm.delete()
            without = r.data != "data_purchased"
            ...
            if without:
                course_batches = "Please provide the batch ID manually for extraction.\n"
            else:
                data = {"user_id": userId}
            ...
            await message.reply_text("⏰ You didn’t reply in time!")
...
async def videocrypt_login(_, message, user_id=None):
    buttons = main_func.get_page(0, core_func.videoCryptDict, DictID="VideoDecrypt1", back_data="tools_", withoutIdPass=False)
    if user_id:
