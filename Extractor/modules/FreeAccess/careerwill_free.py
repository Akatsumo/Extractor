...
async def course_content(base_url, session, cookies, batch_type, batch_id, msg):
    ...
    def fetch_data(session, cookies, batch_type, batch_id, topic_id):
      params.update({"topic_id": topic_id})
      response = session.get(f"{base_url}/class.json", cookies=self.cookies, params=params)
      ...
      for topic in classes_results:
            ...
            if ext == "youtube":
                ...
                if response.json().get("pageProps", {}).get("classDetailsData"):
                   continue
                v_count +=1
                output_link = response.json().get("pageProps", {}).get("classDetailsData").get("lessonUrl", "Not Found")
                ...
                if response.json().get("pageProps", {}).get("classDetailsData"):
                   continue
                v_count +=1
                stream_token = response.json().get("pageProps", {}).get('streamToken').get('token')
                lesson_url = response.json().get("pageProps", {}).get("classDetailsData")[.get('lessonUrl')
                lectures.append(
    ...
    for topic in topic_results:
        ...
        fetch_data(session, cookies, batch_type, batch_id, topic_id)
...
async def careerwill_access(_, message, user_id=None):
    ...
    try:
        msg = await message.reply_text("Fetching All Type of Careerwill Categories. Please Wait...")
        if not token:
        ...
        categorie_list = "📚 **Available Categories:**\n\n"
        for categorie in categories:
            categorie_list += f"`{categorie.get('id')}` - **{categorie.get('batchCat_name')}**\n"
          
        await msg.edit_text(f"{categorie_list}\n**📊 Now send the Categories ID to Download**")       
        input2 = await app.listen(user_id=user_id, timeout=30)
        categorie_id = input1.text.strip()
        await input2.delete()

        batch_type = next((categorie["batchCat_name"] for categorie in categories if str(categorie["id"]) == categorie_id), None)
        if not batch_type:
            return await msg.edit_text("**Invalid Categorie ID. Please try again.**")
      
        params = {"batch_type": batch_type, "view": "Grid", "interface_id": "1", "cat_id": categorie_id}
        ...
        batch_name = next((course["batchName"] for course in data if str(course["id"]) == batch_id), None)
        if not batch_name:
