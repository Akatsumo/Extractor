...
async def course_content(session, batch_id, course_ids):
    ...
    for id in course_ids:
      response_data = session.get(f"https://jchemistry-api.edmingle.com/nuSource/api/v1/public/tutor/class/curriculum/{id}?institution_bundle_id={batch_id}")
      if response_data.status_code != 200:
        continue
        resources_data = response_data.json().get("course_curriculum", {}).get("resources", [])
        if not resources_data:
          continue
        for resourse in resources_data:
          section_name = resources.get("section_name", "N/A")
          for rs in resources.get("resources", []):
            ...
            if material_type == "video/mp4":
              url = vimeo_url if vimeo_url else drm_url if is_drm else vdocipher_video_id if is_enterprise_drm  else  videocrypt_video_id if is_videocrypt_drm else None
              if url:
                v_count += 1
                lecture.append(f"{section_name} | {material_name}: {url}")
              
            if material_type == "application/pdf":
              pdf_url = vimeo_url if vimeo_url else drm_url if is_drm else vdocipher_video_id if is_enterprise_drm  else  videocrypt_video_id if is_videocrypt_drm else None
              if pdf_url:
                p_count += 1
                lecture.append(f"{section_name} | {material_name}: {url}")
...
async def jchemistry_access(_, message, user_id=None):
    ...
    try:
        await msg.edit_text("**Fetching All J Chemistry Batches, Please Wait...**")
        response = session.get("https://jchemistry-api.edmingle.com/nuSource/api/v1/institute/157/courses")
        if response.status_code != 200:
            return await msg.edit_text("Failed to fetch J Chemsitry batches")
        ...
        for course in course_data:
            ...
            if not course_bundle:
              continue
            batch_data.append({"course_id": course_bundle.get('institution_bundle_id'), "name": course_bundle.get('bundle_name'), "course_ids": course_bundle.get('course_ids')})
