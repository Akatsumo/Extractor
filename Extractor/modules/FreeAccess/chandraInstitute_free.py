...
async def course_content(session, headers, userId, batch_id):
    ...
    for subject in subject_data:
        subject_name_raw = subject.get("subject_name", "N/A")
        subject_name = subject_name_raw.split(":")[1].strip() if ":" in subject_name_raw else subject_name_raw
        ...
        response_data = session.post("http://api.chandrainstitute.com/api/v2/api.php/get/class/all/chapters/list", json=data, headers=headers)
        if response_data.status_code != 200:
            continue
        ...
        for chapter in chapter_data:
            ...
            if chapter_type == "LECTURE":
                v_count += 1
                v_url = vimeo_url if vimeo_url else f"https://youtu.be/{main_func.decode_base64(yt_id)}"
...
async def chandraInstitute_access(_, message, user_id=None):
    ...
    try:
        ...
        data = {"user_id": userId, "course_type": "videos", "payment_type": "unpaid"}
        ...
        if len(batch_list) > 4000:
            ...
            with open(fileN, "w", encoding="utf-8") as f:
                f.write(batch_list)
            batch_file = await app.send_document(chat_id=user_id, document=fileN, caption=caption, thumb=thumb)
            os.remove(fileN)
        else:
        ...
        await message.reply_text("⏳ Time Up! Reply fast next time.")
