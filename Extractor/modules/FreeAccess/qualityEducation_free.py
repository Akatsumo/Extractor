    ...
    for item in fetch_data:
        ...
        for subject in subject_data:
          ...
          for topic in topic_data:
            ...
            video_url =  topic.get("quality_1080") if topic.get("quality_1080") else topic.get("quality_720")
            if video_url:
...
async def qualityEducation_access(_, message, user_id=None):
    ...
    try:
        msg = await message.reply_text("Fetching All Batches of Quality Education, Please Wait...")
        response = session.get("https://test.qualityeducation.in/api/video-category-get", headers=headers)
        if response.status_code != 200:
            return await msg.edit_text("Failed to fetch Qualitu Education batches")
        ...
        for course in data:
            batch_list += f"`{course.get('id')}` - **{course.get('category_name')}**\n"
