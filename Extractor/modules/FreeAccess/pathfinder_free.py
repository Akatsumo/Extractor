...
async def course_content(session, batch_id, parent_id):
    ...
    data = {"categoryplanid": batch_id, "loggedinkey": "", "loginid": "", "categoryparentid": parent_id}
    response_data = session.get("http://onlineclass.pathfinderacademy.in/api/category/getcategory", data=data)  
    if response_data.status_code != 200:
        return lectures, v_count, p_count
        
    content_data = response_data.get("message", [])
    if not content_data:
        return lectures, v_count, p_count
    ...
    for content in content_data:
        ...
        if no_of_cat > 0:
            await course_content(session, batch_id, cat_id)
        else:
            data = {"categoryplanid": batch_id, "loggedinkey": "", "loginid": "", "categoryid": cat_id, "categorymainid": parent_id}
            response_data = session.get("http://onlineclass.pathfinderacademy.in/api/video/getvideo", data=data)  
            if response_data.status_code != 200:
              continue
            video_data = response_data.json().get("message", [])
            for video in videos:
...
async def patherfinder_access(_, message, user_id=None):
    ...
    try:
        ...
        category_data = response.get("message", [])
        ...
        batch_data = response.get("message", [])
        if not batch_data:
