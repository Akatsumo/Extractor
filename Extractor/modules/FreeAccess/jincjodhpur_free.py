...
async def jincJodhpur_access(_, message, user_id=None):
    ...
    try:
        ...
        for course in data:
            batch_list += f"`{course.get('id')}` - **{course.get("json", {}).get("ALFvY")}**\n"
        ...
        batch_name, course_section  = next(((course["json"]["ALFvY"], course) for course in data if str(course["id"]) == batch_id), (None, None))
