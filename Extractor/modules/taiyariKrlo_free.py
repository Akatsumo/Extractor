...
async def course_content(session, base_url, batch_id, msg):
    ...
    lesson_data = response.json().get('documents', [])
    if not lesson_data:
        return lectures, v_count, p_count

    for lesson in lesson_data:
      subject_name = lesson.get("fields").get("subject").get("stringValue")
      lesson_name = lesson.get("fields").get("title").get("stringValue")
      ...
      class_data = response.json().get('documents', [])
      if not class_data:
        continue
        
      for class in class_data:
        class_title = class.get("fields").get("title").get("stringValue", "N/A")
        material_type = class.get("fields").get("type").get("stringValue", "N/A")
        
        if material_type == "pdf":
          p_count += 1
          url = class.get("fields").get("ref").get("stringValue", "N/A")
...
async def taiyarKarlo_access(_, message, user_id=None):
    ...
    try:
        ...
        base_url = f"https://firestore.googleapis.com/v1/projects/taiyari-karlo/databases/(default)/documents/courses"
        ...
        batch_name = next((course.get("fields").get('title', {}).get('stringValue', 'N/A') for course in batch_data if str(course.get('name').split('/')[-1]) == batch_id), None)
        if not batch_name:
