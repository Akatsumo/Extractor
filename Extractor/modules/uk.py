...
class UtkarshExtractor:
    ...
    async def process_topic(self, course_id, batch_id, subject_id, topic_id, key, iv):
        ...
        topic_data = self.fetch(f"{self.API_BASE}/data_model/course/get_master_data", 
    ...
    async def process_course(self, batch_id, key, iv):
        lectures = []

        data = {"course_id": batch_id, "parent_id": ""}
        course_data = self.fetch(f"{self.API_BASE}/data_model/course_deprecated/get_course_detail", 
            main_func.encrypt(key, iv, json.dumps(data)), key, iv
        )
        ...
        for tile in course_data["data"].get("tiles", []):
            ...
            for course in tile.get("meta", {}).get("list", []):
                ...
                course_detail = self.fetch(f"{self.API_BASE}/data_model/course_deprecated/get_course_detail",
                    main_func.encrypt(key, iv, json.dumps(data)), key, iv
                )
                ...
                for topic in tile.get('meta', {}).get('list', []):
                    for subtopic in topic.get('list', []):
                        topic_lectures = await self.process_topic(course_id, batch_id, topic.get('id'), 
                            subtopic.get('id'), key, iv
                        )
                        lectures.append(topic_lectures)
    ...
    async def start_login(self, app, message, user_id=None):
        ...
        try:
            ...
            lectures = await asyncio.create_task(self.process_course(batch_id, key, iv))
