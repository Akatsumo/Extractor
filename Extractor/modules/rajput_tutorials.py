...
class Rajput_Tutorials:
    def __init__(self):
        self.v_count = 0
        self.p_count = 0
        self.DEFAULT_BASE = "0117108641864451"
        self.BASE = "1171086418644515_681"
            ...
            if content.get('file_type') == '3':  # Video
                ...
                if content.get('had_pdf') == '1':
                    ...
                    result = await self.fetch_with_retry(
                        session,
                        f"{self.API_BASE}/data_model/poll/get_content_pdf",
                        headers,
                        encrypted_data,
                        key,
                        iv
    ...
    async def process_topic(self, session, course_id, batch_id, topic_id, count, topic_title, subject_id, tile_type, tile_id, revert_api, headers, key, iv):
        try:
            page, total = 1, 0
            contents = ""
            while True:
                ...
                for content in topic_data['data'].get('list', []):
                    if content.get('file_type') in ('3', '1', '7'):
                        ...
                        if url:
                            contents += f"[{topic_title}]-{content['title']} : {url}\n"
                page += 1
    ...
    async def process_course(self, session, course_id, batch_id, headers, key, iv):
        try:
            ...
            results = ""
            for tile in course_detail.get('data', {}).get('tiles', []):
                if tile.get('type') not in ["video", "pdf"]:
                    continue

                for topic in tile.get('meta', {}).get('list', []):
                    for subtopic in topic.get('list', []):
                        results += await self.process_topic(
                            session, course_id, batch_id,
                            subtopic['id'], subtopic['count'], subtopic['title'],
                            topic['id'], tile['type'], tile['id'],
                            tile.get('revert_api'), headers, key, iv
                        )
            return results
        except Exception as e:
    ...
    async def extract_content(self, app, query, message):
        try:
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                ...
                if not courses:
                    return await msg.edit_text("❌ No courses found!")
                ...
                await send_file(filename, caption, msg, query)
...
async def rajput_tutorials(app, query, message):
    extractor = Rajput_Tutorials()
