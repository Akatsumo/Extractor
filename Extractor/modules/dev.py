...
@app.on_edited_message(filters.command(["eval", "m"]) & filters.user(OWNER_ID) & ~filters.forwarded & ~filters.via_bot)
@app.on_message(filters.command(["eval", "m"]) & filters.user(OWNER_ID) & ~filters.forwarded & ~filters.via_bot)
async def executor(client, message):
    ...
    if len(final_output) > 4096:
        ...
        await message.reply_document(document=filename, caption=f"<b>🔗 Eval :</b>\n<code>{cmd[0:980]}</code>\n\n<b>📕 Result :</b>\nAttached document", quote=False, reply_markup=keyboard)
        os.remove(filename)
    else:
        t2 = time()
...
async def forceclose_command(_, CallbackQuery):
    callback_data = CallbackQuery.data.strip().split(None, 1)[1]
    query, user_id = callback_data.split("|")
    if CallbackQuery.from_user.id != int(user_id):
        await CallbackQuery.answer("It's better if you stay in your limits.", show_alert=True)
        return
...
async def shellrunner(_, message):
    ...
    if len(output) > 4096:
        with open("output.txt", "w+") as file:
            file.write(output)
        await _.send_document(message.chat.id, "output.txt", reply_to_message_id=message.id, caption="<code>Output</code>")
...
async def update(_, message):
    ...
    os.execl(sys.executable, sys.executable, "-m", "Restriction")
