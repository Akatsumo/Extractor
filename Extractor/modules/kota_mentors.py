...
class KotaMentors:
    def __init__(self):
        ...
        self.BASE = "1171086418644515_675"
    ...
    async def get_content_url(self, session, headers, content):
        try:
            url = None
            if content.get('file_type') == '3':
                if content.get('is_drm') == '1':
                    ...
                    self.v_count += 1
                #urls = content.get('bitrate_urls', [])
                #if urls:
                #    url = max(urls, key=lambda x: int(''.join(filter(str.isdigit, x['title']))))['url'].replace('\\/', '/').replace('https\\:', 'https:')
                #    self.v_count += 1
                #else:
                #    url = f"https://youtu.be/{content['file_url']}"
                #    self.v_count += 1
        ...
        try:
            ...
            for content in topic_data['data']['list']:
                if content.get('file_type') in ('3', '1', '7'):
                    url = await self.get_content_url(session, headers, content)
                    if url:
...
async def kota_mentors(app, query, message):
    extractor = KotaMentors()
