...
async def course_content(session, scraper, api, message, raw_text2, parent_Id, hdr1, msg): try: response = await session.get(f"https://{api}/get/folder_contentsv2?course_id={raw_text2}&parent_id={parent_Id}", headers=hdr1) if response.status != 200: raise Exception(f"Failed to fetch course content, status code: {response.status}") output = await response.json() data_list = output.get('data', [])
...
async def course_content2(session, scraper, api, message, raw_text2, parent_Id, hdr1, msg, data): global v_count, p_count try: vj = "" tid = data.get("Title", "Unknown Title") lec, vs = "", ""
...
async def appex_v2_txt(app, message, user_id, api, name): global v_count, p_count async with aiohttp.ClientSession() as session: raw_url = f"https://{api}/post/userLogin" hdr = { "Auth-Key": "appxapi", "User-Id": "-2", "Authorization": "", "User_app_category": "", "Language": "en", "Content-Type": "application/x-www-form-urlencoded", "Accept-Encoding": "gzip, deflate", "User-Agent": "okhttp/4.9.1" } info = {"email": "", "password": ""} msg = await message.reply_text("🔑 Send your ID & Password in format: ID*Password")
