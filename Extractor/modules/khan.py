...
class KhanExtractor:
    ...
    async def extract_lessons(self, session, headers, slug):
        try:
            response = await session.get(self.lessons_url.format(slug=slug), headers=headers)
    ...
    async def login(self, session, msg, user_id):
        await msg.edit_text(
        ...
        try:
            ...
            await msg.edit_text(f"⚠️ Error while login: `{e}`")
            return None
...
async def khan_handler(_, message, user_id=None):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            msg = await message.reply_text("🔐 **Initializing Login...**")
            token = await extractor.login(session, msg, user_id)
            if not token:
                return
            headers = {
            ...
            batch_list, batch_map = await extractor.get_batches(session, headers)
            ...
            lectures = await asyncio.create_task(extractor.extract_lessons(session, headers, slug))
            end_time = time.time()
